...
class EfficientTAMCameraPredictor(EfficientTAMBase):
    ...
    def _manage_memory_obj(self, frame_idx, current_out):
        ...
        # 修改这里: 保留前三帧信息
        # 按时间顺序排序所有帧
        sorted_keys = sorted(list(output_dict["non_cond_frame_outputs"].keys()))
        # 获取前三帧的索引
        frames_to_keep = sorted_keys[:3] if len(sorted_keys) >= 3 else sorted_keys
        
        # 如果超过最大记忆帧数量且需要删除
        if len(non_cond_frame_outputs) > self.num_maskmem:
            # 需要删除的帧数
            frames_to_remove = len(non_cond_frame_outputs) - self.num_maskmem
            ...
            for t in sorted_keys:
                # 跳过要保留的前三帧
                if t in frames_to_keep:
                    continue
                    
                # 删除非保留帧的早期帧
                _ = non_cond_frame_outputs.pop(t, None)
                removed_count += 1
