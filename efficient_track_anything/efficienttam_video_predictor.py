...
class EfficientTAMVideoPredictor(EfficientTAMBase):
    ...
    def init_state(
        self,
        video_path=None,  # Optional video path
        live_frame=None,  # Optional single live frame
        offload_video_to_cpu=False,
        offload_state_to_cpu=False,
        async_loading_frames=False,
    ):
        """
        Initialize an inference state. Supports live frames (real-time video) or pre-recorded videos.

        Args:
            video_path (str, optional): Path to an MP4 video or folder of JPEG frames.
            live_frame (np.ndarray, optional): A single frame (H, W, C) for real-time processing.
            offload_video_to_cpu (bool): Whether to offload video frames to CPU memory.
            offload_state_to_cpu (bool): Whether to offload inference states to CPU memory.
            async_loading_frames (bool): Whether to load video frames asynchronously.

        Returns:
            dict: Inference state for tracking.
        """
        compute_device = self.device  # Device of the model
        inference_state = {}

        if live_frame is not None:
            # Handle real-time live frame input
            video_height, video_width = live_frame.shape[:2]
            inference_state["images"] = [torch.from_numpy(live_frame).to(compute_device)]
            inference_state["num_frames"] = 1
        elif video_path is not None:
            # Handle pre-recorded MP4 video or JPEG folder
            ...
            raise ValueError("Either `video_path` or `live_frame` must be provided.")

        # Whether to offload the video frames to CPU memory
        inference_state["offload_video_to_cpu"] = offload_video_to_cpu
        # Whether to offload the inference state to CPU memory
        inference_state["offload_state_to_cpu"] = offload_state_to_cpu
        # Original video height and width, used for resizing final output scores
        ...
        # Frames that already hold consolidated outputs from click or mask inputs
