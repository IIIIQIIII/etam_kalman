...
class EfficientTAMBase(torch.nn.Module):
    ...
    def _forward_sam_heads(
        ...
        """
        Forward SAM prompt encoders and mask heads with Kalman filter tracking.
        ...
        if self.pred_obj_scores:
            lambda_is_obj_appearing = (
                object_score_logits.sigmoid() if self.soft_no_obj_ptr else is_obj_appearing.float()
            )
        ...
        return (
            ...
            obj_ptr,
            object_score_logits,
            ious[0][best_iou_inds] if 'best_iou_inds' in locals() else torch.ones(1, device=device),
    ...
    def _use_mask_as_output(self, backbone_features, high_res_features, mask_inputs):
        ...
        return (
            ...
            object_score_logits,
            # 添加返回值以匹配新的接口
            torch.ones(1, device=mask_inputs.device),
            torch.ones(1, device=mask_inputs.device),
        )
    ...
    def track_step(
        ...
        current_out["best_iou_score"] = best_iou_score  # New
        current_out["kf_score"] = kf_ious  # New
